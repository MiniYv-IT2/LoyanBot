...
> 来源：用户提供的星空月亮飞鸟 404 动画模板（Pug + SCSS）
> 用途：已转换为 React + antd 原生组件（`panel/src/pages/NotFound.jsx`），此文件保留原始代码供随时查看
> 转换要点：SCSS `@for` 循环展开为 JS useMemo 随机生成（内联 style）；`@import` Google Fonts 移除（系统字体替代）；`100vh` 双声明 `100dvh`；文案走 i18n 三语
